...
switch ($_POST["accion"])
{
	case "listar_viaje":
	{
        ...
        $parte= Modelo\BD\ParteLogisticaBD::getParteByTrabajadorFecha($trabajador,$_POST['fecha']);
...
//	}
...

	{
        ...
        $parte = BD\PartelogisticaBD::getParteByTrabajadorFecha($trabajador,$_POST['fecha']);
        ...
        echo "<div class='col-xs-8 col-xs-offset-2'><button type='button' id='salir' class='btn btn-warning salir'>Salir</button></div>";
    ...
    case "generar_calendario":
    {
        ...
        for ($i=1;$i<=$diasdespues;$i++)
        {
            if ($tr<$totalfilas)
            {
                if ($i>=$primeromes && $i<=$tope)
                {
                    ...
                    $trabajador = unserialize($_SESSION["trabajador"]);
                    $parte = Modelo\BD\ParteLogisticaBD::getPartebyTrabajadorFecha($trabajador,$fecha_completa);
                    $hoy = new DateTime();
     ...
     case "addViaje":{
        ...
        if(isset($_SESSION['Parte']) && unserialize($_SESSION["Parte"])->getFecha() == $_POST["fecha"]){
            ...
            $parte= BD\ParteLogisticaBD::getParteByTrabajadorFecha($trabajador,$fecha->format('Y-m-d'));
            if($parte!=null){
